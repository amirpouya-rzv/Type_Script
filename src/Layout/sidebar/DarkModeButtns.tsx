...
const DarkModeButtns = () => {
    ...
    return (
        <div className='flex justify-between md:justify-end md:mt- md:mx-2 items-center  '>
                       ...
                       <button className={`block mt-5 transition-all transform ${them === "dark" ? "rotate-90" : ""}`} 
                       onClick={()=>{dispatch(togleThem())}}>
                       {
                           them === "light" ? <GiMoonBats size={30} className='text-gray-400 mt-3 mx-2'/> : <GiSunflower  size={36} className='text-yellow-300 mt-5 mx-2'/>
                       }
                       </button>
